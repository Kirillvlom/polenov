<?php 
       ...
       $result = mysql_query("SELECT username FROM user WHERE username='$new_username'",$db);
        ...
        '$new_password', view_name = '$new_view_name' WHERE id ='$id'";
          ...
          if($result == mysql_query($query)) {
        	...
        	window.location = \"http://polenov/ws-admin/admin.php\"; </script>");
            } else {
   ...
   $unloading = mysql_query("SELECT * FROM user WHERE id='$id'",$db); //извлекаем из базы все данные о пользователе с введенным логином
    ...
    $_SESSION['user_logo'] = $myrow['user_logo']; 
	    ...
	    exit("<html><head><title>Загрузка..</title><meta http-equiv='Refresh' content='0; URL=ws-admin/admin.php'></head></html>");

} else{
	echo " <script> alert('Нужно заполнить все поля')</script>";
	 exit("<html><head><title>Загрузка..</title><meta http-equiv='Refresh' content='0; URL=admin.php'></head></html>");
}
